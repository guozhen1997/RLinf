...
``ref_chunk`` 或 expert action，并把 ``record_transition``、
...
- Stage 2 actor-critic：
  ...
  - ``train/replay/actor_switch_rate``：收集到的 step 中，route 实际执行 actor 而非 VLA ``ref_chunk`` 的比例（来自 ``forward_inputs.record_transition``，含 schedule warmup 掩码）。
