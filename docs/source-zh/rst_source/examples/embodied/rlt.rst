...
``rlt_switch_flags``、``in_critical_phase`` 和 ``record_transition``。HF rollout
...
Stage 2 数据保持一致。expert 只用于 train rollout；eval rollout 中
``allow_expert=False``，评测的是学到的 actor。
...
.. list-table::
   ...
   * - ``env.*.rlt_policy_switch``
     - 自动产生 ``rlt_switch_flags``、``in_critical_phase`` 和 ``record_transition``。
