...
在模型配置（``examples/steam/value/config/model/steam.yaml``\ ）中设置路径：
...
配置文件为 ``examples/steam/value/config/steam_value_model.yaml``\ ；模型默认值在 ``config/model/steam.yaml``\ 。关键字段：
...
配置文件为 ``examples/steam/process/config/compute_advantages_ensemble.yaml``：
...
.. code-block:: text
   ...
   rlinf/
   ├── models/embodiment/steam/                   # 评论器、集成、配置、合并
