#!/usr/bin/env python3
"""Collect critical-phase PegInsertionSide-v1 demos for ``rlt_maniskill_joint``.
...
4. Save only the grasped near-hole insertion suffix when the full replay succeeds.
...
LOG = logging.getLogger("collect_maniskill_peg_lerobot_joint_critical")

ENV_ID = PEG_INSERTION_SIDE_HARD_ENV_ID
CAMERA_ENV_ID = PEG_INSERTION_SIDE_HARD_OBSERVER_WIDE_WRIST_ENV_ID
...
DEFAULT_CRITICAL_NEAR_HOLE_X_MIN = -0.10
DEFAULT_CRITICAL_NEAR_HOLE_YZ_MARGIN = 2.0
DEFAULT_CRITICAL_PRE_ROLL_FRAMES = 5
DEFAULT_RESET_STATES_SUFFIX = "_critical_reset_states.pt"
...
def _is_critical_phase(info: dict[str, Any], args: argparse.Namespace) -> bool:
    grasp = _bool_scalar(info.get("consecutive_grasp_current", False))
    success = _bool_scalar(info.get("success", False))
    prealigned = _bool_scalar(info.get("prealigned_current", False))
    partial_insert = _bool_scalar(info.get("partial_insert_current", False))
    if not grasp or success:
        return False

    hole_x = _float_scalar(info.get("peg_head_hole_x", np.nan))
    abs_y = _float_scalar(info.get("peg_head_hole_abs_y", np.nan))
    abs_z = _float_scalar(info.get("peg_head_hole_abs_z", np.nan))
    head_yz = _float_scalar(info.get("peg_head_goal_yz_dist", np.nan))
    body_yz = _float_scalar(info.get("peg_body_goal_yz_dist", np.nan))
    hole_radius = _float_scalar(info.get("hole_radii", args.fallback_hole_radius))
    if not np.isfinite(hole_radius) or hole_radius <= 0:
        hole_radius = args.fallback_hole_radius

    yz_error = max(head_yz, body_yz)
    yz_margin = args.critical_near_hole_yz_margin
    near_hole_yz = (
        np.isfinite(yz_error)
        and np.isfinite(abs_y)
        and np.isfinite(abs_z)
        and yz_error <= yz_margin * hole_radius
        and abs_y <= yz_margin * hole_radius
        and abs_z <= yz_margin * hole_radius
    )
    near_hole_x = np.isfinite(hole_x) and hole_x >= args.critical_near_hole_x_min
    return bool(near_hole_x and (near_hole_yz or prealigned or partial_insert))
...
def _reset_states_output_path(repo_id: str, requested_path: str) -> Path:
    if requested_path:
        return Path(requested_path).expanduser()
    dataset_path = _resolve_output_path(repo_id)
    return dataset_path.with_name(f"{dataset_path.name}{DEFAULT_RESET_STATES_SUFFIX}")
...
def _replay_joint_delta_episode(
    *,
...
) -> tuple[list[FrameRecord], list[np.ndarray], int, Any] | None:
    obs, _ = env.reset(seed=seed)
    base_env = getattr(env, "unwrapped", env)
    event_state = init_peg_insertion_event_state(
        num_envs=1,
        device=getattr(base_env, "device", "cpu"),
    )
    reset_peg_insertion_event_state(event_state)
    replay_records = [_extract_record(obs)]
    replay_actions: list[np.ndarray] = []
    replay_pre_action_states: list[Any] = []
    last_info: dict[str, Any] = {}
    critical_start_idx: int | None = None
    critical_reset_state: Any | None = None

    for step_idx, solver_action in enumerate(solver_actions):
        curr_qpos = replay_records[-1].qpos
        state_before_action = env.get_state()
        replay_pre_action_states.append(state_before_action)
        ...
        last_info = _augment_replay_info(env, info, event_state)
        replay_actions.append(action.astype(np.float32))
        replay_records.append(_extract_record(obs))

        if critical_start_idx is None and _is_critical_phase(last_info, args):
            critical_start_idx = max(
                0,
                len(replay_actions) - args.critical_pre_roll_frames,
            )
            critical_reset_state = replay_pre_action_states[critical_start_idx]

        if _bool_scalar(last_info.get("success", False)):
            break
    ...
    if not _bool_scalar(last_info.get("success", False)):
        ...
        return None
    if critical_start_idx is None:
        LOG.info("Rejecting seed %d: replay succeeded but never entered critical phase", seed)
        return None
    if critical_reset_state is None:
        LOG.info("Rejecting seed %d: missing critical reset state", seed)
        return None
    if len(replay_actions) - critical_start_idx < args.min_critical_steps:
        LOG.info(
            "Rejecting seed %d: critical phase has only %d steps",
            seed,
            len(replay_actions) - critical_start_idx,
        )
        return None
    return replay_records, replay_actions, critical_start_idx, critical_reset_state


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Collect successful hard-clearance PegInsertionSide-v1 critical-phase "
            "demos as a replayable rlt_maniskill_joint LeRobot dataset."
        )
    )
    parser.add_argument("--repo-id", default="local/rlt_maniskill_joint_critical")
    ...
    parser.add_argument(
        "--critical-near-hole-x-min",
        type=float,
        default=DEFAULT_CRITICAL_NEAR_HOLE_X_MIN,
    )
    parser.add_argument(
        "--critical-near-hole-yz-margin",
        type=float,
        default=DEFAULT_CRITICAL_NEAR_HOLE_YZ_MARGIN,
    )
    parser.add_argument(
        "--critical-pre-roll-frames",
        type=int,
        default=DEFAULT_CRITICAL_PRE_ROLL_FRAMES,
    )
    parser.add_argument("--min-critical-steps", type=int, default=2)
    parser.add_argument("--fallback-hole-radius", type=float, default=0.03)
    parser.add_argument(
        "--solver-control-mode",
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Output .pt file for critical reset states. Defaults to "
            "<dataset_path>_critical_reset_states.pt."
        ),
    )
...
def main() -> None:
    ...
    reset_states_path = _reset_states_output_path(args.repo_id, args.reset_states_path)
    ...
    if reset_states_path.exists() and args.overwrite:
        LOG.info("Removing existing reset states at %s", reset_states_path)
        reset_states_path.unlink()
    ...
    try:
        while saved < args.num_episodes and attempts < args.max_attempts:
            ...
            replay_records, replay_actions, critical_start_idx, critical_reset_state = replay
            critical_records = replay_records[critical_start_idx:]
            critical_actions = replay_actions[critical_start_idx:]
            frames = _build_frames(
                records=critical_records,
            ...
            reset_states.append(
                {
                    "episode_index": saved,
                    "seed": episode_seed,
                    "critical_start_idx": critical_start_idx,
                    "full_steps": len(replay_actions),
                    "saved_steps": len(critical_actions),
                    "state": critical_reset_state,
                }
            )
            LOG.info(
                "Saved critical episode %d from seed %d: full_steps=%d, "
                "critical_start=%d, saved_steps=%d",
                saved,
                episode_seed,
                len(replay_actions),
                critical_start_idx,
    ...
    reset_states_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(
        {
            "env_id": ENV_ID,
            "camera_env_id": CAMERA_ENV_ID,
            "control_mode": args.target_control_mode,
            "critical_near_hole_x_min": args.critical_near_hole_x_min,
            "critical_near_hole_yz_margin": args.critical_near_hole_yz_margin,
            "critical_pre_roll_frames": args.critical_pre_roll_frames,
            "states": reset_states,
        },
        reset_states_path,
    )
    LOG.info("Saved %d successful episodes after %d attempts to %s", saved, attempts, output_path)
    LOG.info("Saved %d critical reset states to %s", len(reset_states), reset_states_path)
