...
class FlowMatchingActionHeadForRLActionPrediction(nn.Module):
    ...
    def sample_mean_var_val(
        ...
        action_features = (
            self.action_encoder(x_t, timesteps_tensor, embodiment_id)
            if hasattr(self, "action_encoder")
            else x_t
        )
        if getattr(self.config, "add_pos_embed", False) and hasattr(
        ...
        v_t = (
            self.action_decoder(model_output, embodiment_id)
...
class GR00T_N1_6_ForRLActionPrediction(Gr00tN1d6, BasePolicy):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        action_head_cfg = (
            config.action_head_cfg if hasattr(config, "action_head_cfg") else config
        )
    ...
    def _load_metadata(self, exp_cfg_dir: Path):
        ...
        if not metadata_path.exists():
            print(
                f"⚠️ Metadata file not found at {metadata_path}. Inferring from modality_config..."
            )

            tag_value = self.embodiment_tag.value
            if self._modality_config is None or tag_value not in self._modality_config:
                print(
                    f"⚠️ Modality config is missing or does not contain tag {self.embodiment_tag.value}. Attempting to infer valid_action_dim and image_nums from config attributes."
                )
                self.valid_action_dim = getattr(
                ...
                print(
                    f"✅ Inferred fallback: valid_action_dim={self.valid_action_dim}, image_nums={self.image_nums}"
                )
                return
            ...
            if "action" in current_modality:
                ...
                if (
                    ...
                    if (
                        "dim_map" in action_modality_cfg
                        and action_modality_cfg["dim_map"]
                    ):
                        for dim_val in action_modality_cfg["dim_map"].values():
                            valid_action_dim += dim_val
            ...
            print(
                f"✅ Inferred from modality_config: valid_action_dim={self.valid_action_dim}, image_nums={self.image_nums}"
            )
            return
