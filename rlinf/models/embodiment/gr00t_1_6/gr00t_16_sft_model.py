...
class GR00T_1_6_SFT_Model(Gr00tN1d6, BasePolicy):
        ...
        **kwargs,
        ...
        if local_model_path is None:
            local_model_path = getattr(
                config, "_name_or_path", "/workspace/RLinf/GR00T-N1.6-3B"
            )
        ...
        if hasattr(self, "action_head") and self.action_head is not None:
            ...
            if hasattr(self.action_head, "model") and hasattr(
                self.action_head.model, "timestep_encoder"
            ):
                self.action_head.model.timestep_encoder.requires_grad_(False)
                print(
                    "Precisely disabled timestep_encoder gradient updates (to prevent NaN)"
                )
    ...
    def sft_forward(self, data: dict, **kwargs):
        ...
        padded_actions = torch.zeros(
            (actions.shape[0], actions.shape[1], target_dim),
...
def build_gr00t_dataloader(worker_instance, eval_dataset: bool = False):
    ...
    print(
        "[Lazy Load] Successfully triggered GR00T model file registration! Loading LeRobot Dataset..."
    )
