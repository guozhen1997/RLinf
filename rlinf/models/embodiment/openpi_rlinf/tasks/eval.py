...
def env_done_mask(
    dones: Any | None, batch_size: int, device: torch.device
) -> torch.Tensor | None:
    """Collapse a batched done flag to ``(batch,)`` bool on ``device``."""
    if dones is None:
        return None
    mask = torch.as_tensor(dones, device=device)
    ...
    if mask.shape != (batch_size,):
        raise ValueError(
            f"SFP dones must broadcast to shape {(batch_size,)}; got {tuple(mask.shape)}."
        )
    return mask
...
class Pi0Eval(EnvIO, Pi0):
    ...
    def predict_action_batch(
        ...
        **kwargs,
        ...
        if self.sfp_cfg.use_sfp:
            ...
            return self._predict_sfp_eval(env_obs, dones=dones)
    ...
    def _predict_sfp_eval(
        self, env_obs: dict[str, Any], *, dones: Any | None
    ) -> tuple[torch.Tensor, dict[str, Any]]:
        from rlinf.models.embodiment.openpi_rlinf.sampling.sfp_sampler import (
            sample_sfp_actions,
        )

        batch_size = self._sfp_batch_size(env_obs)
        self._prepare_sfp_action_states(batch_size, dones)
        observation = self.env_obs_to_observation(env_obs)
    ...
    def _prepare_sfp_action_states(self, batch_size: int, dones: Any | None) -> None:
        ...
        reset = env_done_mask(dones, batch_size, device)
