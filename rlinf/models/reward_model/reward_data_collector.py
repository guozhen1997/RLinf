...
class RewardDataCollector:
    ...
    def _extract_success_once_list(self, env_info: Dict, batch_size: int) -> List[Optional[bool]]:
    ...
    def _update_trajectory_success_once(self, env_idx: int, success_once: Optional[bool], 
                                            env_success_frame, buffer_is_empty: bool):
    ...
    def _process_env_step(self, env_idx: int, obs, env_success_frame, dones,
                         success_once: Optional[bool], batch_size: int):
        ...
        self._update_trajectory_success_once(env_idx, success_once, env_success_frame, buffer_is_empty)
        ...
        if is_env_done:
            ...
            if buffer_len <= 5:
                print(f"WARNING: Saving trajectory with only {buffer_len} steps for env {env_idx}")
        ...
        for env_idx in list(self.trajectory_buffers.keys()):
            if len(self.trajectory_buffers[env_idx]) > 0:
                ...
                if last_step_done:
                    ...
                    if buffer_len <= 5:
                        print(f"WARNING: Saving trajectory with only {buffer_len} steps for env {env_idx} (at epoch end)")
                    self.save_trajectory(env_idx)
    ...
    def add_env_output(
        ...
        for env_idx in range(batch_size):
            env_success_frame = self._extract_env_success_frame(success_frame, env_idx, batch_size)
            self._process_env_step(
                env_idx, obs, env_success_frame, dones,
                success_once_list[env_idx], batch_size
            )
    ...
    def _collect_trajectory_data(self, trajectory_buffer: List[Dict], env_idx: int, is_positive: bool) -> Dict:
        ...
        for step_idx, step_data in enumerate(trajectory_buffer):
            ...
            step_labels = self._extract_step_labels(success_frame, step_idx, env_idx, is_positive)
    ...
    def save_trajectory(self, env_idx: Optional[int] = None):
        ...
        collected = self._collect_trajectory_data(trajectory_buffer, env_idx, is_positive)
    ...
    def _extract_image(self, image, key_offset=0):
        ...
        if isinstance(image, torch.Tensor):
            ...
            if image_np.ndim == 3:
                ...
                print(f"WARNING: Unexpected image shape after batch extraction: {image_np.shape}")
                return None
        else:
