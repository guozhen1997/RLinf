...
class ManiskillEnv(gym.Env):
    ...
    def _wrap_obs(self, raw_obs):
        # Tonghe corrected: use unwrapped to access the obs_mode attribute.
    ...
    def _extract_obs_image(self, raw_obs):
        ...
        proprioception: torch.Tensor = self.env.unwrapped.agent.robot.get_qpos().to(obs_image.device)  # qpos. To see the link to the definition of get_qpos(), remove the ".unwrapped" and click onto the function. 
        extracted_obs["state"] = proprioception
    ...
    def step(
        ...
        if self.is_start:
            # print(f"step::self.use_fixed_reset_state_ids={self.use_fixed_reset_state_ids}")
            # print(f"step::self.reset_state_ids={self.reset_state_ids}")
            ...
            # print(f"""step::options={input_options}""")
