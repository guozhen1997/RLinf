...
from rlinf.data.image_processing import get_visual_transform
import numpy as np
import torch
...
from rlinf.data.energon.chatml import ChatMLSample

from megatron.training import get_args
from rlinf.tokenizer import get_tokenizer
...
class TaskEncoder(DefaultTaskEncoder[Union[VQASample, ChatMLSample], ImageTaskSample, VQATaskBatch, dict]):
    ...
    def batch(self, samples: List[ImageTaskSample]) -> VQATaskBatch:
        ...
        batch = VQATaskBatch(
            ...
            __key__ = samples[0].__key__ if samples else str(uuid.uuid()),
            __restore_key__ = samples[0].__restore_key__ if samples and hasattr(samples[0], '__restore_key__') else str(uuid.uuid4()),
        )
