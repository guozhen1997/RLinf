...
def validate_sft_cfg(cfg: DictConfig) -> DictConfig:
    ...
    with open_dict(cfg):
        ...
        eval_data_paths = cfg.data.get("eval_data_paths", None)
        has_eval_data = val_data_paths is not None or eval_data_paths is not None
        if cfg.data.get("train_data_paths", None) is None:
            # if train_data_paths is None, the code will just eval the model
            assert has_eval_data, (
                "the data.train_data_paths is None, so data.val_data_paths "
                "or data.eval_data_paths is required"
            )
        elif has_eval_data:
