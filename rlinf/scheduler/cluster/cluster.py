...
class Cluster:
    ...
    def _looks_like_ray_duplicate_named_actor_error(exc: BaseException) -> bool:
        ...
        return (
            ...
            or ("actor" in msg and "name" in msg and ("taken" in msg or "collision" in msg))
        )
        ...
        if num_nodes is not None or cluster_cfg is not None:
            ...
            while True:
                try:
                    ...
                    if self._ray_instance_count > Cluster._MAX_NAMESPACE_CONFLICT_RETRIES:
                        raise RuntimeError(
    ...
    def _paths_equivalent_for_code_sync(left: str, right_canonical: str) -> bool:
        ...
        try:
            ...
            return os.path.normcase(os.path.abspath(os.path.expanduser(left))) == os.path.normcase(
                os.path.abspath(right_canonical)
            )
    ...
    def _resolve_explicit_abs_path_to_repo_and_rlinf(
        ...
        if (
            ...
            and (resolved / "rlinf" / "__init__.py").is_file()
        ):
    ...
    def _maybe_stage_git_tracked_rlinf_py_modules_dir(
        ...
        try:
            ...
            logger.warning("git executable not found; syncing the full local rlinf/ tree.")
            return local_rlinf
    ...
    def _strip_sync_roots_from_pythonpath(
        ...
        kept = [
            ...
            if not any(
                cls._paths_equivalent_for_code_sync(e, r) for r in strip_roots
            )
        ]
        if kept:
    ...
    def _init_and_launch_managers(
        ...
        self._ray_code_sync_fragment, self._runtime_code_sync_strip_roots = (
            Cluster._prepare_ray_code_sync_runtime_env_fragment(self._num_nodes)
        )
